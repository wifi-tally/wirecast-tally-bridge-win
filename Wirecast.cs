...
namespace WirecastTallyBridge {
    ...
    /// !!! WARNING !!!
    ///
    /// This class is a 1-to-1 copy from the WirecastCSharp example and could be a license problem.
    ///
    /// As it is labled "example" it is probably fine, but better be on the save side.
    ///
    ...
    public class WirecastBinding
    {
        ...
        public void DebugPrint(int menu_id, int indent)
        {
            string name = (string)Late.Invoke(_Document, "MenuName", menu_id);
            bool isChecked = (bool)Late.Invoke(_Document, "MenuChecked", menu_id);

            for (int idx = 0; idx < 2 * indent; idx++)
            {
                Console.Write(" ");
            }
            Console.Write(menu_id);
            if (isChecked)
            {
                Console.Write(" *");
            }
            ...
            {
                int child_menu_id = (int)Late.Invoke(_Document, "MenuByIndex", menu_id, idx);
                if (child_menu_id == 0)
                    break;
                DebugPrint(child_menu_id, indent + 1);
            }
        }
...
//            DebugPrint(0, 0);
            ...
            if (output_menu_id != 0)
            {
                //int ndi_menu_id = (int)Late.Invoke(_Document, "MenuByIndex", output_menu_id, 12);
                int ndi_menu_id = (int)Late.Invoke(_Document, "MenuByName", output_menu_id, "NDI� Output");
                if (ndi_menu_id != 0)
                {
                    //int run_menu_id = (int)Late.Invoke(_Document, "MenuByIndex", ndi_menu_id, 3);
                    int run_menu_id = (int)Late.Invoke(_Document, "MenuByName", ndi_menu_id, "480p25");
                    if (run_menu_id != 0)
                    {
                        Late.Invoke(_Document, "ExecuteMenu", run_menu_id);
                    }
                }
            }
        }
